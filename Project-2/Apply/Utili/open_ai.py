...
import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), 'Apply', 'Utili', 'Functions')))
from Apply.Utili.Functions import *  # Ensure all required functions are imported
from Apply.Utili.Functions import generate_duckdb_query  # Explicitly import the missing function
from Apply.Utili.Functions import transcribe_youtube_segment
load_dotenv()

AIPROXY_TOKEN = os.getenv("AIPROXY_TOKEN")
if not AIPROXY_TOKEN:
    raise ValueError("AIPROXY_TOKEN environment variable is not set. Please check your .env file.")
...
async def get_openai_response(question: str, file_path: Optional[str] = None) -> str:
    ...
    if (
        ...
        from Apply.Utili.Functions import convert_keyvalue_to_json
    ...
    if (
        ...
        from Apply.Utili.Functions import process_encoded_files
